...
class OrderItemDao {
  ...
  def insert(orderItemRecord: OrderItemRecord)(implicit session: DBSession, ec: ExecutionContext): Future[Int] =
    futureUpdate {
  ...
  def findByOrderId(orderId: String)(implicit session: DBSession, ec: ExecutionContext): Future[List[OrderItemRecord]] =
    Future {
      withSQL {
